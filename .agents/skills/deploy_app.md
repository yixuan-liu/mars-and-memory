...
2. **Install Dependencies**: Use your native terminal to navigate into `app_build/` and run `pnpm install`, `pip install -r requirements.txt`, or whatever is appropriate!
3. **Host Locally**: Execute the appropriate native terminal command (e.g., `pnpm run dev`, `python3 app.py`) to start a background server.
